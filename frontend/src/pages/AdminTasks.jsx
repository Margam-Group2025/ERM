...
export default function AdminTasks() {
  ...
  useEffect(() => {
    const fetchTeamLeads = async () => {
      const res = await fetch("https://erm-rust.vercel.app/api/users?role=teamlead", {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const res = await fetch(
        "https://erm-rust.vercel.app/api/tasks/team?assignedToRole=teamlead",
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://erm-rust.vercel.app/api/tasks", {
        method: "POST",
        headers: {
